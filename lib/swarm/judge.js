...
// ── The AI judge ─────────────────────────────────────────────────────────────
export function judgePrompt({ text, kind, title, business, people, question = "", market = "", plan = "" }) {
...
FROM: ${business}${plan ? `
${plan}
These people are the ones described in that plan, and most have never heard of this kind of product. Judge whether the text reaches THEM — someone busy with their own business who is not shopping for this — not whether it is well written.` : ""}${market ? `\nTHE PEOPLE ARE IN: ${market}` : ""}${question ? `\nTHE OWNER WANTS TO KNOW: ${String(question).slice(0, 300)} (answer this in each person's sentence)` : ""}
${title ? `TITLE: ${title}\n` : ""}THE TEXT:
"""
