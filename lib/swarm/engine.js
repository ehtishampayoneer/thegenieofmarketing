...
async function buyersFor(admin, userId, host, ctx) {
  ...
  if (!buyers && !ctx.aiDown) {
    ...
    const trusted = learned?.weights && learned.cv
      ...
      : [];
    const r = await ai_(ctx, { prompt: crowdPrompt(ai, host, voices, trusted), maxTokens: 5000, temperature: 0.9, userId, host });
  ...
  const out = { buyers: buyers || fallbackCrowd(ai), mode, cv, business: ai.businessName || host };
...
export async function testItem(admin, item, ctx) {
  ...
  const { buyers, mode: crowdMode, business, cv } = await buyersFor(admin, userId, host, ctx);
  ...
  const crowd = {
    ...
    am: mode === "rules" ? undefined : Object.fromEntries(people.filter((p) => !p.gate).map((p) => [p.id, Math.round(meanOf(reactions[p.id]?.dist || [0, 0, 1, 0, 0]) * 100) / 100])),
