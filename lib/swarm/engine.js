...
export async function buyersFor(admin, userId, host, ctx, { market = "" } = {}) {
  ...
  if (!buyers && !ctx.aiDown) {
    ...
    const trusted = learned?.weights && learned.cv
      ...
      : [];
    const r = await ai_(ctx, { prompt: crowdPrompt(ai, siteHost, voices, trusted, market), maxTokens: 5000, temperature: 0.9, userId, host });
...
export async function runCrowd(admin, ctx, { userId, host, kind, text, title = "", meta = "", seedKey, market = "", question = "", improve = "auto", previous = null }) {
  ...
  if (!ctx.aiDown) {
    ...
    for (let i = 0; i < people.length; i += BATCH) {
      ...
      const r = await ai_(ctx, { prompt: judgePrompt({ text, kind, title, business, people: batch, question, market }), maxTokens: 3000, temperature: 0.9, userId, host });
      if (!r) break;
  ...
  if (mode !== "rules" && wantImprove && !ctx.aiDown) {
    ...
    const w = await ai_(ctx, {
      prompt: improvePrompt({ text: isArticle ? meta : text, kind, title, business, tickets, quotes, isArticle }),
...
export async function testItem(admin, item, ctx) {
  ...
  await recordEvent(admin, {
    ...
    data: { itemId: item.id, source: item.source, kind, score: crowd.score, size: crowd.size, mode: crowd.mode, improved: !!improvement, tickets: before.objections.length },
  });
  if (improvement) {
    await recordEvent(admin, {
...
export function newContext(admin, budgetMs = 230000) {
  return { admin, crowds: new Map(), aiDown: !freeProvidersReady().length, deadline: Date.now() + budgetMs, calls: 0 };
}
