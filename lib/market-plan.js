...
// Best to hardest, for a list an owner reads top-down.
const TIER_RANK = { green: 0, amber: 1, red: 2 };

/**
 * How hard this market is. Market Testing's own verdict where it has one — it
 * judges difficulty from how saturated and how reachable a country is, which is a
 * better answer than re-deriving it from the opportunity score. A country the
 * owner typed in by hand has neither, and comes back as the middle tier rather
 * than as the hard one.
 */
export function tierFor(market) {
  const d = String(market?.difficulty || "").toLowerCase();
  if (d === "easy") return TIERS[0];
  if (d === "medium") return TIERS[1];
  if (d === "hard") return TIERS[2];
  return tierOf(market?.score);
}
...
export function activeMarkets(markets = [], dailyCap = 5, { max = MAX_ACTIVE } = {}) {
  const rows = (Array.isArray(markets) ? markets : [])
    ...
    .filter((m) => m.name)
    .map((m) => ({ ...m, tier: tierFor(m) }))
    .sort((a, b) => (TIER_RANK[a.tier.id] ?? 1) - (TIER_RANK[b.tier.id] ?? 1) || (Number(b.score) || 0) - (Number(a.score) || 0))
  ...
  const out = rows.map((r, i) => {
    ...
    given += n;
    return { ...r, tier: r.tier.id, label: r.tier.label, why: r.tier.why, score: r.score ?? null, emailsToday: n };
  });
...
/**
 * Which market tonight's article is written for.
 *
 * NOT "the same article again in another country's coat". Two near-identical pages
 * differing only in the currency are doorway pages, which is the exact thing
 * Google's scaled-content policy penalises, and the publish guard already refuses
 * them. So the market decides WHO a piece is for and which of the plan's keywords
 * is worth writing next — never that a written piece gets rewritten per country.
 *
 * Rotates on the same weights the sending uses, so over six articles a green
 * market gets three, an amber two and a hard one one.
 *
 * @param markets from activeMarkets()
 * @param counts  { [market name]: how many articles it already has }
 */
export function nextMarket(markets = [], counts = {}) {
  const rows = (Array.isArray(markets) ? markets : []).filter((m) => m?.name);
  if (!rows.length) return null;
  // A count read from the database can come back null, missing or as text.
  const had = counts && typeof counts === "object" ? counts : {};
  let best = null;
  let bestRatio = Infinity;
  for (const m of rows) {
    const weight = SHARE[m.tier] ?? 0.2;
    const seen = Math.max(0, Number(had[m.name]) || 0);
    // How over-served this market already is, against the share it is owed.
    const ratio = (seen + 1) / weight;
    if (ratio < bestRatio - 1e-9) { best = m; bestRatio = ratio; }
  }
  return best || rows[0];
}
