import { ownerSignalLearnings } from "@/lib/owner-signal";
...
export function synthesizeLearnings({ decisions = [], placements = [], keywords = [], conversions = [] }) {
  ...
  // 6) WHAT THE OWNER SKIPS AND REWRITES — the only signal in the product that
  //    comes from the owner's own hands rather than from a number. Kept in its own
  //    module because both of its consumers live elsewhere (lib/owner-signal.js).
  learnings.push(...ownerSignalLearnings(decisions));
