 ...
 */
export function brainBlock({ strategy = null, ai = null, targets = null, audience = null } = {}) {
  const parts = [];

  const s = strategy ? normalizeStrategy(strategy) : null;
  if (s && strategyReady(s)) parts.push(strategyBlock(s, { audience }));
 ...
 * @param {string}  [opts.audience]     "partner" to write for an agency who would
 *                                      resell this, rather than for a buyer
 * @returns {Promise<{ready, strategy, ai, host, businessName, markets, targets, block}>}
 */
export async function genieBrain(supabase, { userId, host = null, ai = null, draft = false, targets = 0, audience = null } = {}) {
  const base = {
    ...
    if (!checked || Date.now() - checked > MARKETS_STALE_DAYS * 864e5) {
      try {
        ...
        strategy = normalizeStrategy({
          ...strategy,
          markets: fresh.length ? fresh.map((m) => m.name) : strategy.markets,
          // How hard each one is, not only its name — this is what makes the
          // queue's green, amber and red mean three different things.
          marketData: fresh.length ? fresh : strategy.marketData,
  ...
  return {
    ...
    targets: picked,
    audience,
    block: brainBlock({ strategy: norm, ai: scanAi, targets: picked, audience }),
  };
}
