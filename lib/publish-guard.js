...
export async function checkScaledContent(supabase, { userId, host, title, body, excludeActionId = null } = {}) {
  ...
  try {
    // ── NOT ITSELF ──
    // This compared the article being published against every article the owner
    // has, INCLUDING the one being published. So every piece matched itself at
    // 100%, every publish was refused as a near-copy of a title identical to its
    // own, and no article could ever go live. The blog stayed empty for weeks and
    // the reason read like a content problem.
    let q = supabase
      .from("actions").select("id, title, payload")
      .eq("user_id", userId).eq("type", "article");
    if (excludeActionId) q = q.neq("id", excludeActionId);
    const { data: prior } = await q.order("created_at", { ascending: false }).limit(25);

    const mine = shingles(text);
    if (mine.size === 0) return out;
    for (const a of prior || []) {
      if (excludeActionId && a?.id === excludeActionId) continue; // belt and braces
...
export async function guardContent(supabase, { userId, host, channel, content, entity, title, deep = false, excludeActionId = null } = {}) {
  ...
  if (["blog", "article", "answer", "website", "wordpress"].includes(channel)) {
    scaled = await checkScaledContent(supabase, { userId, host, title, body: text, excludeActionId });
