...
export function conversionCtaHtml({ cta, url, businessName = "", keyword = "" }) {
  ...
  return (
    ...
    `<p style="margin:0 0 16px;font-size:14.5px;line-height:1.5;color:#475569">${esc(c.subtext)}</p>` +
    `<a href="${url}" rel="noopener" style="display:inline-block;padding:12px 24px;border-radius:999px;background:#0f172a;color:#ffffff;font-weight:600;font-size:14.5px;text-decoration:none">${esc(c.buttonText)} &rarr;</a>` +
    `</div>`
  );
}
