...
// ── PLAIN-TEXT CLEANER ──
// For text that is POSTED AS-IS (social posts, Reddit/Quora, outreach, pitches) — not
// run through markdownToHtml. Strips the raw markdown symbols that scream "AI wrote
// this": # headings, **/* emphasis, `code`, - / * bullets, and em-dashes. Preserves
// real content: #hashtags (a # glued to a word), in-word hyphens (well-known), numbered
// lists, and links (kept as "text (url)"). Never throws.
export function cleanText(s) {
  if (typeof s !== "string") return s;
  let t = s;
  t = t.replace(/```[a-z0-9]*\n?/gi, "").replace(/`([^`]+)`/g, "$1").replace(/`/g, ""); // code fences + inline code
  t = t.replace(/^\s{0,3}#{1,6}\s+/gm, "");                 // "## Heading" -> "Heading" (keeps #hashtags: no space)
  t = t.replace(/^\s{0,3}>\s?/gm, "");                       // blockquote markers
  t = t.replace(/^\s{0,3}[-*+]\s+/gm, "• ");                 // "- item" / "* item" -> "• item" (removes the minus/asterisk)
  t = t.replace(/\[([^\]]+)\]\((https?:\/\/[^)\s]+)\)/g, "$1 ($2)"); // [text](url) -> text (url)
  t = t.replace(/\*\*([^*]+)\*\*/g, "$1").replace(/__([^_]+)__/g, "$1"); // bold
  t = t.replace(/(^|[^*\w])\*([^*\n]+)\*/g, "$1$2").replace(/(^|[^_\w])_([^_\n]+)_/g, "$1$2"); // italic
  t = t.replace(/\*\*/g, "").replace(/(?<!\w)_{2,}(?!\w)/g, ""); // stray leftover markers
  t = deDash(t);                                            // em/en dashes -> natural punctuation
