...
// Real per-COUNTRY performance (impressions/clicks/position) for a property the account
// owns — the honest backbone for market expansion. Country keys are ISO-3166-1 alpha-3
// (e.g. "usa", "arg", "geo"). Returns { available:false } when the site isn't a property.
// The Search Console property that covers this host, or null. Shared so every
// reader matches properties the same way.
export async function resolveGscProperty(accessToken, host) {
  try { return matchProperty(await listSites(accessToken), host); } catch { return null; }
}
export { query as gscQuery, daysAgo as gscDaysAgo };

export async function getGscCountries(accessToken, host) {
  let sites;
  try { sites = await listSites(accessToken); } catch { return { available: false, reason: "error" }; }
  const site = matchProperty(sites, host);
  if (!site) return { available: false, reason: "no_property" };
  const endDate = daysAgo(3), startDate = daysAgo(90);
  let res;
  try { res = await query(accessToken, site, { startDate, endDate, dimensions: ["country"], rowLimit: 250 }); }
  catch { return { available: false, reason: "error" }; }
  const rows = (res.rows || []).map((r) => ({
    code: String(r.keys[0] || "").toLowerCase(),
    clicks: Math.round(r.clicks), impressions: Math.round(r.impressions),
    ctr: +(r.ctr * 100).toFixed(1), position: +r.position.toFixed(1),
  })).filter((r) => r.code && r.impressions > 0);
  return { available: true, site, dateRange: { startDate, endDate }, countries: rows };
}
