...
/**
 * The buyer questions we ask the models.
 *
 * `planQueries` is the plan's "where they already look", which the owner can
 * correct on /strategy. It comes first: re-deriving these from the scan every
 * night meant a correction the owner made in one place was ignored here, and the
 * wedge measured visibility for questions their buyers do not actually ask.
 */
export function aiSearchQuestions(entity, ai = {}, keywords = [], planQueries = []) {
  const derived = buildIntentQueries(entity, ai, keywords);
  if (!planQueries?.length) return derived.slice(0, 6);
  ...
  for (const q of planQueries) {
    ...
    // Stage/weight match what buildIntentQueries gives a growth-intent query, so
    // the score is computed the same way whichever list a question came from.
    out.push({ query, stage: "growth", weight: 3, group: "unnamed" });
    if (out.length >= 3) break;
  }
  for (const d of derived) {
    if (out.length >= 6) break;
    if (seen.has(String(d.query || "").toLowerCase())) continue;
    seen.add(String(d.query || "").toLowerCase());
    out.push(d);
  }
