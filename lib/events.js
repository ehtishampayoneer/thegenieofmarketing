...
export async function getEvents(supabase, { userId = null, host = null, types = null, since = null, limit = 200 } = {}) {
  try {
    // `id` comes back too: the live views key rows by it, and an event's id is
    // how the launch test reopens a past run.
    let q = supabase.from("events").select("id, type, actor, subject, data, host, created_at").order("created_at", { ascending: false }).limit(limit);
