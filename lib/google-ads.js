...
export async function adsKeywordIdeas(token, customerId, keywords, version = workingVersion || VERSIONS[0]) {
  const res = await fetch(`${apiBase(version)}/customers/${customerId}:generateKeywordIdeas`, {
    ...
    body: JSON.stringify({ keywordSeed: { keywords }, keywordPlanNetwork: "GOOGLE_SEARCH", geoTargetConstants: ["geoTargetConstants/2840"], language: "languageConstants/1000" }),
  });
  ...
  return { ok: true, volume: Number(first?.keywordIdeaMetrics?.avgMonthlySearches || 0) };
}
