...
export async function runEntityPipeline(admin, appUrl, { userId, host }) {
  ...
  // And work nobody was ever going to reach. Genie writes every night; the queue
  // asks for three. Without this the difference just accumulates, and a queue that
  // only grows reads to the owner as a debt they are behind on rather than a day's
  // work they have finished.
  let expired = 0;
  try {
    const { expireStaleDrafts } = await import("@/lib/stuck");
    expired = (await expireStaleDrafts(admin, { userId, host })).expired;
  } catch {}
  ...
  const metrics = {
    dispatched, retired, escalated, pillared, proposed, expired,
    readyBefore: before,
    skipped,
    ms: clock.elapsed(),
