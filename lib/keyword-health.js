...
// Roll per-keyword nightly Search Console snapshots into one portfolio climb:
// average Google position per day (LOWER is better) plus clicks. Real data only —
// fewer than two days with a position means there's no climb to show yet, and we
// return null rather than invent a curve.
export function climbFrom(series) {
  const byDay = {};
  for (const rows of Object.values(series || {})) {
    for (const r of rows || []) {
      if (r?.position == null) continue;
      const d = (byDay[r.date] ||= { date: r.date, sum: 0, n: 0, clicks: 0 });
      d.sum += Number(r.position) || 0;
      d.n += 1;
  ...
  const points = Object.values(byDay)
    ...
    .map((d) => ({ date: d.date, position: d.sum / d.n, clicks: d.clicks }));
  if (points.length < 2) return null;
  const first = points[0].position;
  const last = points[points.length - 1].position;
  return {
    points, first, last,
    // Positive delta = climbed (the position number fell).
    delta: Math.round((first - last) * 10) / 10,
    clicks: points.reduce((s, p) => s + p.clicks, 0),
  };
}
