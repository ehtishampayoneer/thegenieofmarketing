...
// Fetch a site's homepage and return its usable images. Best-effort, short timeout.
export async function harvestSiteImages(hostOrUrl, { limit = 24 } = {}) {
  const origin = siteOrigin(hostOrUrl);
  if (!origin) return [];
  try {
    const res = await fetch(origin, { headers: { "User-Agent": UA, Accept: "text/html" }, signal: AbortSignal.timeout(8000) });
    if (!res.ok) return [];
    const html = (await res.text()).slice(0, 700000);
    return extractImages(html, origin).slice(0, limit);
  } catch { return []; }
}
