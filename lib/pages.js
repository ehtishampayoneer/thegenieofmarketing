...
// Update a hosted page IN PLACE (same handle/slug/URL) — used by the refresh loop
// so a re-optimized article keeps its ranking equity instead of forking a new URL.
export async function updateHostedPage(supabase, { pageId, userId, title, bodyHtml, metaDescription, heroImage, heroAlt, faq }) {
  const patch = { updated_at: new Date().toISOString() };
  if (title) patch.title = title;
  if (bodyHtml) patch.body_html = bodyHtml;
  if (metaDescription != null) patch.meta_description = metaDescription || null;
  if (heroImage !== undefined) patch.hero_image = heroImage || null;
  if (heroAlt !== undefined) patch.hero_alt = heroAlt || null;
  if (Array.isArray(faq)) patch.faq = faq.length ? faq : null;
  const { data, error } = await supabase.from("published_pages").update(patch).eq("id", pageId).eq("user_id", userId).select("id, handle, slug").maybeSingle();
  if (error || !data) throw new Error(error?.message || "hosted_update_failed");
  return { id: data.id, handle: data.handle, slug: data.slug, url: pageUrl(data.handle, data.slug) };
}
