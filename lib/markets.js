...
export function scoreMarkets({ entity = {}, profile = {}, gsc = null }) {
  ...
  const rows = COUNTRIES.map((co) => {
    ...
    // language suitability: English-only can't fairly serve low-English markets you don't localize for
    const supportsLang = langs.includes(co.lang);
    const englishSuitable = co.eng >= ENGLISH_OK;
    const needsLocalLang = eligible && !supportsLang && !englishSuitable;
    ...
    const local = clamp01(co.eng * 0.8 + (supportsLang ? 0.35 : 0)) * (needsLocalLang ? 0.5 : 1);
    ...
    else if (co.eng > 0.7 && !supportsLang) why.push("High English proficiency");
    ...
    return {
      ...
      confidence, bucket, eligible, blockedReason, needsLocalLang, supportsLang, englishSuitable,
