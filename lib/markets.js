...
export const LANG_NAMES = { en: "English", de: "German", nl: "Dutch", ar: "Arabic", pl: "Polish", pt: "Portuguese", ro: "Romanian", el: "Greek", cs: "Czech", ka: "Georgian", uk: "Ukrainian", tr: "Turkish", es: "Spanish", id: "Indonesian", vi: "Vietnamese", ms: "Malay", fr: "French", it: "Italian", ja: "Japanese", ko: "Korean" };

// English content only fairly reaches a mainstream audience where English proficiency is
// high; below this, a market needs local-language content (don't ship a thin English page).
const ENGLISH_OK = 0.62;
...
export function scoreMarkets({ entity = {}, profile = {}, gsc = null }) {
  ...
  const langs = (profile.languages && profile.languages.length ? profile.languages : ["en"]).map((s) => String(s).toLowerCase());
  // HARD, stored eligibility gates
  const delivery = profile.delivery || (audience === "local" ? "local" : "anywhere"); // anywhere | regions | local
  const regions = profile.regions || [];
  const payment = profile.payment || "global"; // global | limited
  const payCountries = (profile.paymentCountries || []).map((s) => String(s).toLowerCase());
  const excluded = (profile.excluded || []).map((s) => String(s).toLowerCase());
  const englishConfirmed = !!profile.englishConfirmed;
  const localOnly = delivery === "local";
  ...
  const rows = COUNTRIES.map((co) => {
    ...
    // ── hard eligibility (gate out, never a low score) ──
    let eligible = true, blockedReason = "";
    if (localOnly) { eligible = false; blockedReason = "local-only"; }
    else if (excluded.includes(co.code)) { eligible = false; blockedReason = "excluded"; }
    else if (delivery === "regions" && !regions.includes(co.region)) { eligible = false; blockedReason = "outside delivery area"; }
    else if (payment === "limited" && !payCountries.includes(co.code)) { eligible = false; blockedReason = "no payment coverage"; }
    // Language suitability. High English proficiency alone is NOT enough to auto-approve
    // English content for a non-native market — the user must confirm English is acceptable
    // there, or there must be real evidence (existing English GSC impressions from that
    // country). Otherwise it's "English test possible", not fully targetable.
    const supportsLang = langs.includes(co.lang);
    const nativeEn = co.lang === "en";
    const highEnglish = co.eng >= ENGLISH_OK;
    const hasEnglishEvidence = !!(g && g.impressions > 0);
    let langState;
    if (nativeEn || supportsLang) langState = "native";
    else if (!highEnglish) langState = "needs_local";
    else if (englishConfirmed || hasEnglishEvidence) langState = "native";
    else langState = "english_test";
    const needsLocalLang = eligible && langState === "needs_local";
    const englishTest = eligible && langState === "english_test";
    const targetable = eligible && langState === "native";
    ...
    if (g && g.position >= 8 && g.position <= 30) gap = clamp01(gap + 0.18);
    const fit = eligible ? clamp01(co.pay * (0.6 + (audience === "global" ? 0.4 : audience === "b2b" ? 0.3 : 0.25))) : 0;
    const local = clamp01(co.eng * 0.8 + (supportsLang ? 0.35 : 0)) * (langState === "needs_local" ? 0.5 : langState === "english_test" ? 0.8 : 1);
    const searchTraction = g ? clamp01(g.clicks > 0 ? 0.3 + Math.min(0.7, g.clicks / 50) : 0.05) : 0;
    const opp = eligible ? clamp01(fit * 0.3 + gap * 0.25 + demand * 0.2 + local * 0.15 + searchTraction * 0.1) : 0;
    ...
    // PROJECTED monthly organic visitors at maturity (a projection — labelled as such on the page)
    const expTraffic = g ? Math.round(g.impressions * 0.3) : Math.round(Math.sqrt(co.net) * (0.4 + demand * 0.6) * rankProb * 55);
    // sales are ESTIMATES with an explicit assumed conversion → shown as a range, never a promise
    const convEff = conv * (co.emg ? 1.3 : 1) * (0.7 + co.pay * 0.3);
    const salesMid = expTraffic * convEff;
    const expSalesLow = Math.max(0, Math.round(salesMid * 0.6));
    const expSalesHigh = Math.max(expSalesLow, Math.round(salesMid * 1.6));
    const assumedConvPct = Math.round(convEff * 1000) / 10; // e.g. 2.3(%)
    const days = difficulty === "Easy" ? 45 : difficulty === "Medium" ? 75 : 120;
    // progress is SEARCH progress only until lead/revenue attribution exists
    ...
    if (needsLocalLang) why.push(`Needs ${LANG_NAMES[co.lang] || co.lang} content`);
    else if (englishTest) why.push("English test possible");
    else if (hasEnglishEvidence && !nativeEn && !supportsLang) why.push("Already gets English search traffic here");
    if (supportsLang && co.lang !== "en") why.push("You support the local language");

    return {
      code: co.code, iso2: co.iso2, flag: flagEmoji(co.iso2), name: co.name, region: co.region, lang: co.lang, langName: LANG_NAMES[co.lang] || co.lang,
      demand: Math.round(demand * 100), gap: Math.round(gap * 100), fit: Math.round(fit * 100), local: Math.round(local * 100),
      opp: Math.round(opp * 100), difficulty, expTraffic, expSalesLow, expSalesHigh, assumedConvPct, days, progress,
      confidence, bucket, eligible, targetable, blockedReason, langState, needsLocalLang, englishTest, supportsLang, nativeEn, hasEnglishEvidence,
      gsc: g, why: why.slice(0, 3),
    };
  });

  rows.sort((a, b) => b.opp - a.opp || b.expSalesHigh - a.expSalesHigh);
