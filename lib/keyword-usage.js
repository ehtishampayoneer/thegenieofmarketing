...
export async function selectTargets(supabase, userId, host, { count = 1 } = {}) {
  ...
  // AI-search citation gaps (source:'aeo') that aren't covered yet jump the queue —
  // when a real AI engine recommends a rival instead of you, winning that answer page
  // back is the highest-leverage thing Genie can write. Turns the AI-search REPORT
  // into an immediate FIX in Approvals rather than an eventual one.
  const aeoGap = (k) => (k.source === "aeo" && (k.coverage || 0) === 0) ? 1 : 0;

  const ranked = [...keywords].sort((a, b) => {
    const ba = boostFor(a), bb = boostFor(b);
    if (ba !== bb) return bb - ba;                       // MONEY first — what converts wins
    const ga = aeoGap(a), gb = aeoGap(b);
    if (ga !== gb) return gb - ga;                       // then win back AI citations
...
// ── GIVING A TOPIC BACK ──
// Coverage advances the moment an article is DRAFTED, which is right: it stops the
// same keyword being written twice while the first one waits in Approvals. But it
// also means that when a draft is thrown away — Genie discarding its own near-copy,
// say — the keyword stays marked as covered forever and that topic is silently lost
// from the plan. The owner never finds out; the article simply never gets written.
export async function releaseUsage(supabase, userId, host, { refId, primary } = {}) {
  if (!supabase || !userId || !host) return;
  // Drop the ledger rows so the "used in" trail does not point at deleted work.
  if (refId) {
    try { await supabase.from("keyword_usage").delete().eq("user_id", userId).eq("ref_id", refId); } catch {}
  }
  const kw = String(primary || "").toLowerCase().trim();
  if (!kw) return;
  try {
    const { data } = await supabase.from("keywords").select("coverage")
      .eq("user_id", userId).eq("host", host).eq("keyword", kw).maybeSingle();
    const next = Math.max(0, (Number(data?.coverage) || 0) - 1);
    await supabase.from("keywords").update({ coverage: next })
