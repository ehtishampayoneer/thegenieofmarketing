...
export async function recordUsage(supabase, { userId = null, host = null, kind, provider, model, tokensIn = 0, tokensOut = 0, latencyMs = 0, cached = false, ok = true, tag = null, costUsd: knownCost = null } = {}) {
  // Always metered — use the request client when present (per-user attribution),
  // else the system client so no call is ever silently unmetered.
  let client = supabase;
  if (!client) { try { client = createAdminClient(); } catch { return 0; } }
  // The router passes the real price for paid calls (from OpenRouter's catalogue).
  // The table below only knows a few models and prices the rest at $0, which is
  // how the daily spend cap used to miss real spending.
  const costUsd = cached ? 0 : Number.isFinite(knownCost) && knownCost != null ? Math.round(knownCost * 1e6) / 1e6 : estimateCost(model, tokensIn, tokensOut);
