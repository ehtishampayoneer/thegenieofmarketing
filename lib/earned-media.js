...
import { NO_BLANKS_RULE, fillBlanks } from "@/lib/fill-blanks";
import { callAI } from "@/lib/ai-router";
import { safeFetch } from "@/lib/ssrf";

// Used by the broken-link checks below. It was referenced but never defined, so
// findDeadLinks threw a ReferenceError on its first request, the catch turned that
// into "no dead links", and the play never found one.
const UA = "Mozilla/5.0 (compatible; MarketingGenieBot/1.0; +https://thegenieofmarketing.vercel.app)";
// Link text without markup. Also referenced below and never defined, which would
// have thrown out of the whole discovery request once a page's links were read.
...
// Matched against the whole domain (or a subdomain of it). This used to be bare
// fragments, so "x\.com" also matched vox.com and netflix.com, and "t\.co"
// matched hubspot.com: real publishers were silently dropped from Get featured.
const MEDIA_SKIP = /(^|\.)(facebook\.com|instagram\.com|twitter\.com|x\.com|t\.co|linkedin\.com|youtube\.com|youtu\.be|tiktok\.com|pinterest\.[a-z.]+|reddit\.com|quora\.com|amazon\.[a-z.]+|ebay\.[a-z.]+|etsy\.com|aliexpress\.com|walmart\.com|target\.com|google\.[a-z.]+|bing\.com|duckduckgo\.com|yahoo\.com|wikipedia\.org|[a-z0-9-]+\.gov(\.[a-z]{2})?|w3\.org|vimeo\.com|dailymotion\.com|rumble\.com|odysee\.com|bitchute\.com|veoh\.com|metacafe\.com|vidlii\.com)$/i;
...
async function pitchMedia(items, play, business) {
  ...
  const arr = await aiList({
    system: `You are Genie's earned-media outreach writer. GOAL: ${spec.label}. For EACH site, write a short, specific, non-spammy outreach email. Approach: ${spec.angle}. Never invent an email not in that site's list. No em-dashes, no emoji, no hype. ${NO_BLANKS_RULE} Return ONLY a JSON array, one object per site, same order.`,
...
[{ "i": 0, "summary": "1 sentence on what this site is / covers", "name": "editor/contact name if evident else null", "title": "their role or a best-guess like 'Editor'", "recommendedEmail": "best email from THAT site's list or null", "whyFit": "1 sentence why we're a genuine fit for their audience", "subject": "specific subject under 60 chars", "body": "warm 60-100 word email for the goal above, one clear value point, one soft ask" }]`,
  ...
  return items.map((it, i) => {
    ...
    return {
      ...
      whyFit: p.whyFit || "",
      // Whatever the model left blank is filled from what is already known here:
      // the contact's name came back with the pitch, the sender's is the business.
      // A pitch reaching the owner as "Hi [Editor Name]" hands the work straight
      // back, on the one screen that exists to say Genie did it.
      pitch: fillPitch(
        {
...
Thanks for your time.`,
        ...
        { recipientName: p.name || null, senderName: business?.name || null, companyName: business?.name || null, recipientCompany: it.company || null }
      ),
    };
  });
}
...
export async function findDeadLinks(pageUrl, { max = 25 } = {}) {
  let html = "";
  try {
    // safeFetch returns { res, finalUrl }, not the response itself. Reading r.ok and
    // r.status off the wrapper was always undefined, so this play never found a
    // single dead link.
    const { res: r } = await safeFetch(pageUrl, { headers: { "User-Agent": UA }, signal: AbortSignal.timeout(12000) });
    if (!r?.ok) return [];
    html = await r.text();
  } catch { return []; }
  ...
  for (let i = 0; i < candidates.length; i += 5) {
    ...
    const results = await Promise.allSettled(slice.map(async (c) => {
      try {
        let r = (await safeFetch(c.url, { method: "HEAD", headers: { "User-Agent": UA }, signal: AbortSignal.timeout(8000) }))?.res;
        // Some servers reject HEAD outright; confirm with a GET before judging.
        if (r && (r.status === 405 || r.status === 501)) {
          r = (await safeFetch(c.url, { headers: { "User-Agent": UA }, signal: AbortSignal.timeout(8000) }))?.res;
        }
...
export async function diagnoseMedia(play, niche) {
  ...
  // Free providers first; the OpenRouter credit only if they all fail. This used to
  // start with paid gpt-4o-mini through an override the spend cap never counted.
  let sites = await run("chain", {});
  if (!sites.length) sites = await run("or-paid", { only: ["openrouter-paid"] });
  return { probes, mapped: sites.length, sites };
}

// Both halves of a pitch, cleaned the same way. A blank in a subject line is the
// one a recipient sees before they open anything, so it is never forgotten here.
function fillPitch(pitch, who) {
  return { subject: fillBlanks(pitch.subject, who).text, body: fillBlanks(pitch.body, who).text };
}
