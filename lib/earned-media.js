...
export async function findMediaSites(play, niche, { limit = 10 } = {}) {
...
// in one batched call. Reusable so the route can recover from the diagnostic's sites.
...
export async function discoverMedia({ play = "backlinks", niche, business, limit = 8 }) {
  ...
  return { opportunities, debug: { sites: sites.length, reachable: opportunities.filter((o) => o.contact?.email || o.contact?.contactForm).length } };
}

// Diagnostic + recovery: re-run the site-finding call and return both the metadata
// (for the UI) and the actual sites. Same trick that made Find clients reliable — the
// candidate call reliably returns sites at this calmer moment, so the route builds
// prospects from them instead of showing an empty page.
export async function diagnoseMedia(play, niche) {
  ...
  try {
    ...
    const sites = mapCompanies(list);
    return { ai: sites.length ? "ok" : "empty", parsed: (list || []).length, mapped: sites.length, sites };
  } catch (e) {
    return { ai: "failed", error: String(e?.message || e).slice(0, 160), sites: [] };
  }
}
