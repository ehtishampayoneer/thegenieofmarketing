...
export async function findLinkTo(siteUrl, target, { paths = LIKELY_PATHS } = {}) {
  ...
  for (const path of paths) {
    let html = "";
    try {
      // safeFetch returns { res, finalUrl }. Treating the wrapper as the response
      // made r.ok always undefined, so no page was ever read and no link found.
      const { res: r } = await safeFetch(origin + path, { headers: { "User-Agent": UA }, signal: AbortSignal.timeout(10000) });
      if (!r?.ok) continue;
      html = await r.text();
    } catch { continue; }
