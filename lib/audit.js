...
export async function runAudit(rawUrl) {
  ...
  try {
    ...
    // SSRF-guarded fetch: validates the URL and every redirect hop.
    ...
    return {
      ok: false,
      reason: e?.ssrf ? "blocked_url" : (e.name === "AbortError" ? "timeout" : "fetch_failed"),
      url,
      detail: e.message,
    };
  }
  ...
  // Read what a buyer reads, not the menu. Before this, the text was the raw body
  // cut at 3000 characters, and on a typical landing page the first few hundred
  // are the navigation repeated twice. On arqr360.com the prices start at
  // character 3797, so Genie never saw them and asked the owner for prices that
  // were printed on the page. Key pages the homepage links to (pricing, about,
  // services) are read too, because many sites keep prices off the homepage.
  const homeText = extractText($);
  const extra = await fetchKeyPages($, finalUrl, homeText);
  const pageText = [homeText, ...extra].join(" ").slice(0, PAGE_TEXT_MAX);
...
async function fetchSiteFiles(finalUrl) {
  ...
  const grab = async (path) => {
    try {
      ...
      const { res: r } = await safeFetch(origin + path, {
...
export const PAGE_TEXT_MAX = 9000;
const BREAK = String.fromCharCode(10);

// Runs last on the parsed page: it removes elements, and every signal has
// already been extracted by the time it is called.
export function extractText($) {
  $("script, style, noscript, svg, template, iframe").remove();
  $("nav, header, footer, [role=navigation], [role=banner], [role=contentinfo], [aria-hidden=true]").remove();
  $("[class*=cookie], [id*=cookie], [class*=consent], [id*=consent]").remove();
  // Block elements become line breaks, so repeated fragments can be spotted.
  $("p, li, h1, h2, h3, h4, h5, h6, div, section, article, tr, br").each((_, el) => { $(el).append(BREAK); });
  const root = $("main").length ? $("main") : $("body");
  const seen = new Set();
  const lines = [];
  for (const raw of root.text().split(BREAK)) {
    const line = raw.replace(/\s+/g, " ").trim();
    if (!line) continue;
    const key = line.toLowerCase();
    // Short repeated fragments are menus, buttons and badges. Long repeats are
    // kept once. Either way the same words are not paid for twice.
...
// Follow at most two same-site links whose path says pricing, plans, about or
// services. Same-page anchors (#pricing) are skipped, their text is already in.
const KEY_PAGE = /(pric|plan|package|rates|cost|about|services|what-we-do|solutions|how-it-works)/i;
async function fetchKeyPages($, finalUrl, homeText) {
  let origin, home;
  try { const u = new URL(finalUrl); origin = u.origin; home = u.pathname.replace(/\/$/, ""); } catch { return []; }
  const wanted = [];
  $("a[href]").each((_, a) => {
    ...
    if (u.origin !== origin || u.pathname.replace(/\/$/, "") === home) return;
    if (!KEY_PAGE.test(u.pathname)) return;
    const clean = origin + u.pathname;
    if (!wanted.includes(clean)) wanted.push(clean);
  });
  // Pricing first: it is the thing owners are most annoyed to be asked for.
  wanted.sort((a, b) => Number(/pric|plan|package|rates/i.test(b)) - Number(/pric|plan|package|rates/i.test(a)));
  const pages = await Promise.all(wanted.slice(0, 2).map(async (link) => {
    try {
      const ctrl = new AbortController();
      const t = setTimeout(() => ctrl.abort(), 6000);
      const { res: r } = await safeFetch(link, { headers: { "User-Agent": UA, Accept: "text/html" }, signal: ctrl.signal });
      clearTimeout(t);
      if (!r.ok || !/html/i.test(r.headers.get("content-type") || "html")) return "";
      const text = extractText(cheerio.load(await r.text()));
      if (!text || homeText.includes(text.slice(0, 200))) return "";
      return `Page ${new URL(link).pathname}: ${text.slice(0, 3500)}`;
    } catch { return ""; }
  }));
