...
export async function buildPillar(supabase, userId, host, ai = {}) {
  ...
  // THE PLAN. A pillar page is the most authoritative thing Genie publishes, and
  // it used to be written from a business name and one scan field — so the hub
  // could frame the cluster differently from every article inside it.
  const brain = await genieBrain(supabase, { userId, host, ai });
  const bizName = brain.businessName;
