...
import { NO_BLANKS_RULE } from "@/lib/fill-blanks";
...
async function batchPitch(items, userBusiness) {
  ...
  const arr = await aiList({
    system: `You are Genie's B2B prospecting analyst. For EACH company, identify the best decision-maker and write a short, genuinely personalised outreach email from the sender. Never invent an email not in that company's provided list. ${NO_BLANKS_RULE} Return ONLY a JSON array, one object per company, same order.`,
