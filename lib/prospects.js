...
// 0: a provider (seen with Gemini 2.5-flash) sometimes returns a valid-but-EMPTY
// {"companies":[]}, which callAI treats as success and never fails over — so we got
// an empty list even though Groq returns the real companies fine. Fix: if the first
// attempt yields nothing usable, retry forcing the providers that answer reliably.
async function aiList(opts) {
  const attempt = async (only) => firstArray((await callAI({ ...opts, ...(only ? { only } : {}) }))?.json);
  try {
    let list = await attempt(null);
    if (!list.length) list = await attempt(["groq", "openrouter"]);
    return list;
  } catch {
    try { return await attempt(["groq", "openrouter"]); } catch { return []; }
  }
}
