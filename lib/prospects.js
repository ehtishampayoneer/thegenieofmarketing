...
// Run a JSON list-generating AI call ROBUSTLY. The bug that made Find clients return
// 0: Gemini 2.5-flash intermittently returns a valid-but-EMPTY {"companies":[]},
// which callAI treats as success and never fails over. Groq returns the real list
// reliably (proven by the in-UI diagnostic). So: try providers one at a time —
// Groq, then OpenRouter, then Gemini — and take the FIRST non-empty result. Each
// attempt swallows its own error so one bad/rate-limited provider can't abort the
// chain, and a provider that isn't configured is simply skipped.
async function aiList(opts) {
  const attempt = async (only) => { try { return firstArray((await callAI({ ...opts, only }))?.json); } catch { return []; } };
  for (const only of [["groq"], ["openrouter"], ["gemini"]]) {
    const list = await attempt(only);
