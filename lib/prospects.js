...
async function aiList(opts) {
  ...
  if (!list.length) { await sleep(1200); list = await attempt(["groq", "openrouter"]); }
  if (!list.length) { await sleep(1500); list = await attempt(["openrouter"]); } // paid-only, no free-tier limits
