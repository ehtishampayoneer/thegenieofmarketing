...
// When web search is blocked (datacenter IPs frequently are — Vercel included), it
// falls back to an LLM that NAMES real companies + their domains; those are never
// trusted blindly — every candidate is later verified by crawling its own site for a
// real deliverable contact, so a wrong or dead domain is simply dropped.
export async function findCompanies(niche, { limit = 6, ctx = null } = {}) {
  ...
  // Search came back thin/blocked → let the model propose real companies. Free, no
  // key, and reachable from any server (unlike scraping a search engine).
  if (out.length < limit) {
    for (const c of await llmCandidateCompanies(niche, { limit: limit * 2 })) {
      if (out.length >= limit * 2) break;
      if (seen.has(c.domain) || SKIP_HOST.test(c.domain)) continue;
      seen.add(c.domain);
      out.push(c);
    }
  }
