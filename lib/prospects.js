...
export async function buildProspectsFromCompanies(companies, userBusiness, limit = 8, { needsPlatform = null, rivalCat = null } = {}) {
  ...
  for (let i = 0; i < companies.length; i += BATCH) {
    ...
    const rows = await Promise.all(batch.map(async (c) => {
      ...
      return { company: c.name, domain: c.domain, url: c.url, origin, emails: p?.emails || [], text: p?.text || "", contactForm: p?.contactForm || null, platform: p?.platform || null, ar: p?.ar || null, rivalTech: p?.rivalTech || null };
    }));
    profiles.push(...rows);
  }
