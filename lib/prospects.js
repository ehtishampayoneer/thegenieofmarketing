...
import { detectPlatform, platformFit, detectArTech, arAngle, byArOpportunity, readableForAr } from "@/lib/platform-detect";
...
export async function profileCompany(url) {
  ...
  const ar = detectArTech(home);
  return {
    origin, emails: [...emails.values()], contactForm,
    text: text.trim().slice(0, 6000), siteName: ogTitle.trim(),
    platform, ar: arAngle(ar, platform, { readable: readableForAr(home) }), arTech: ar,
  };
}
...
export async function buildProspectsFromCompanies(companies, userBusiness, limit = 8, { needsPlatform = null } = {}) {
  ...
  for (let i = 0; i < companies.length; i += BATCH) {
    ...
    const rows = await Promise.all(batch.map(async (c) => {
      ...
      return { company: c.name, domain: c.domain, url: c.url, origin, emails: p?.emails || [], text: p?.text || "", contactForm: p?.contactForm || null, platform: p?.platform || null };
    }));
    profiles.push(...rows);
  }
...
export async function discoverProspects({ niche, userBusiness, limit = 8, ctx = null, fit = null, needsPlatform = null }) {
  const companies = await findCompanies(niche, { limit, ctx, fit });
  const prospects = await buildProspectsFromCompanies(companies, userBusiness, limit, { needsPlatform });
