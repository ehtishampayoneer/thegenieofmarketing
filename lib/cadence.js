...
export function buildTapPlan(placements) {
  ...
  // Daily ritual order: publish to YOUR OWN accounts first (builds credibility),
  // then join community conversations. Within each group, a friendly order.
  const ORDER = { x: 1, linkedin: 2, medium: 3, wordpress: 4, blog: 4, reddit: 10, quora: 11, forum: 12, guest: 13 };
  blocks.sort((a, b) => {
    // Owned (your accounts) come first.
    if (a.owned !== b.owned) return Number(b.owned) - Number(a.owned);
