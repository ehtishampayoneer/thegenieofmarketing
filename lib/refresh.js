...
export async function refreshStalePage(supabase, { userId, host = null, pageId = null, minStaleDays = 0 } = {}) {
  ...
  // THE PLAN. A refresh rewrites a page that is already ranking, so it is the
  // easiest place to drift: it used to see only the old text and the keyword, and
  // could reframe the business away from the plan every other engine follows.
  const { block: plan } = await genieBrain(supabase, { userId, host: page.host || host });
