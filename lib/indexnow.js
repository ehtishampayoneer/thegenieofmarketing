...
// keyLocation: IndexNow only trusts a key file on the same host as the URLs, and a
// key in a folder only covers that folder. Articles on the owner's /blog pass the
// key file served at /blog/indexnow-key.txt (see app/b).
export async function pingIndexNow(urls, { keyLocation = null } = {}) {
  ...
  try {
    ...
    const res = await fetch("https://api.indexnow.org/indexnow", {
      ...
      body: JSON.stringify({
        host: first.host,
        key,
        keyLocation: keyLocation || `${first.origin}/api/indexnow/key`,
