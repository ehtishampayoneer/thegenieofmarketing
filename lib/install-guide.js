 ...
 */
export function wordpressPlugin({ src, site = "", token = "", appUrl = "" }) {
  const safeSrc = String(src).replace(/'/g, "");
  const safeToken = String(token).replace(/'/g, "");
  const safeApp = String(appUrl || "").replace(/'/g, "").replace(/\/+$/, "");
  return `<?php
/**
 * Plugin Name: Marketing Genie
 * Description: Counts visits, catches leads, points visitors at your buy page, and connects this site to Marketing Genie in one click so approved articles can publish themselves.
 * Version: 1.1.0
...
define( 'MARKETING_GENIE_TOKEN', '${safeToken}' );
define( 'MARKETING_GENIE_APP', '${safeApp}' );
...
/**
 * ONE-CLICK CONNECT.
 * Connecting used to mean the owner creating an "application password" by hand in
 * WordPress and pasting three values into Genie. Most business owners have never
 * heard of application passwords. WordPress can create one itself, so this page
 * does that and sends it to Genie: one button, no copying.
 *
 * Only an administrator can press it, it is nonce-protected, and the password it
 * creates is named so it can be found and revoked in Users -> Profile at any time.
 */
/**
 * Google Search Console verification, handled for the owner.
 * Genie asks Google for the verification tag and posts it here; this prints it in
 * <head> so Google can confirm the owner owns the site. Without this the owner
 * has to paste a meta tag into their theme by hand, which is where most people
 * give up and end up with estimated rankings forever.
 */
add_action( 'wp_head', function () {
    $c = get_option( 'marketing_genie_site_verification' );
    if ( $c ) { echo '<meta name="google-site-verification" content="' . esc_attr( $c ) . '" />' . "\\n"; }
}, 1 );

add_action( 'rest_api_init', function () {
    register_rest_route( 'marketing-genie/v1', '/verification', array(
        'methods'             => 'POST',
        'permission_callback' => function () { return current_user_can( 'manage_options' ); },
        'callback'            => function ( $request ) {
            $content = sanitize_text_field( $request->get_param( 'content' ) );
            if ( ! $content ) { return new WP_Error( 'genie_no_content', 'No verification content.', array( 'status' => 400 ) ); }
            update_option( 'marketing_genie_site_verification', $content );
...
add_action( 'admin_menu', function () {
    add_menu_page( 'Marketing Genie', 'Marketing Genie', 'manage_options', 'marketing-genie', 'marketing_genie_page', 'dashicons-megaphone', 80 );
} );

function marketing_genie_page() {
    if ( ! current_user_can( 'manage_options' ) ) { return; }
    $connected = get_option( 'marketing_genie_connected_at' );
    $notice = '';

    if ( isset( $_POST['marketing_genie_connect'] ) && check_admin_referer( 'marketing_genie_connect' ) ) {
        $result = marketing_genie_connect();
        $notice = is_wp_error( $result )
            ? '<div class="notice notice-error"><p>' . esc_html( $result->get_error_message() ) . '</p></div>'
            : '<div class="notice notice-success"><p>Connected. Approved articles can now publish to this site.</p></div>';
        $connected = get_option( 'marketing_genie_connected_at' );
    }

    echo '<div class="wrap"><h1>Marketing Genie</h1>' . $notice;
    if ( $connected ) {
        echo '<p>This site is connected to Marketing Genie (since ' . esc_html( $connected ) . ').</p>';
        echo '<p>To disconnect, delete the "Marketing Genie" application password in Users &rarr; Profile.</p>';
    } else {
        echo '<p>Press the button and this site is connected to Marketing Genie. Nothing is published without your approval.</p>';
    }
    echo '<form method="post">';
    wp_nonce_field( 'marketing_genie_connect' );
    echo '<p><button class="button button-primary" name="marketing_genie_connect" value="1">' . ( $connected ? 'Reconnect' : 'Connect to Marketing Genie' ) . '</button></p>';
    echo '</form></div>';
}

function marketing_genie_connect() {
    if ( ! MARKETING_GENIE_TOKEN || ! MARKETING_GENIE_APP ) {
        return new WP_Error( 'genie_no_token', 'This plugin copy has no Genie token. Download it again from Marketing Genie.' );
    }
    if ( ! class_exists( 'WP_Application_Passwords' ) ) {
        return new WP_Error( 'genie_old_wp', 'This needs WordPress 5.6 or newer.' );
    }
    if ( ! wp_is_application_passwords_available() ) {
        return new WP_Error( 'genie_app_pw_off', 'Application passwords are disabled on this site (they need HTTPS, or a plugin has turned them off).' );
    }

    $user = wp_get_current_user();
    // Replace any previous one so reconnecting cannot pile them up.
    foreach ( WP_Application_Passwords::get_user_application_passwords( $user->ID ) as $pw ) {
        if ( isset( $pw['name'] ) && $pw['name'] === 'Marketing Genie' ) {
            WP_Application_Passwords::delete_application_password( $user->ID, $pw['uuid'] );
        }
    }
    $created = WP_Application_Passwords::create_new_application_password( $user->ID, array( 'name' => 'Marketing Genie' ) );
    if ( is_wp_error( $created ) ) { return $created; }
    $password = $created[0];

    $res = wp_remote_post( MARKETING_GENIE_APP . '/api/connect/wordpress/plugin', array(
        'timeout' => 20,
        'headers' => array( 'Content-Type' => 'application/json' ),
        'body'    => wp_json_encode( array(
            'token'       => MARKETING_GENIE_TOKEN,
            'siteUrl'     => home_url(),
    ...
    if ( $code !== 200 || empty( $body['ok'] ) ) {
        return new WP_Error( 'genie_rejected', isset( $body['error'] ) ? $body['error'] : ( 'Marketing Genie refused the connection (HTTP ' . $code . ').' ) );
    }
    update_option( 'marketing_genie_connected_at', current_time( 'mysql' ) );
    return true;
}
`;
}
