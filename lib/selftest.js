...
export const CHECKS = [
  ...
  },
  {
    ...
    run: async (ctx) => {
      ...
      if (!b.block) return fail("The brain returned nothing, so engines are writing with no business context at all.", "Send this report to your developer.");
...

  {
    ...
    run: async (ctx) => {
      ...
      if (!last) return warn(`The crowd has not tested anything yet. ${capacity}.`, "It runs after tonight's nightly run. For testing all day, add the heartbeat: GitHub repo Settings → Secrets and variables → Actions → secret HEARTBEAT_SECRET (same as HEARTBEAT_SECRET in Vercel) and variable GENIE_URL.");
  ...
  // ── OUTREACH SAFETY AND PACE ──
  {
    ...
    run: async (ctx) => {
      ...
      const { data, error } = await ctx.supabase.from("directory_contacts")
        .select("email, source, industry, company").eq("user_id", ctx.userId).limit(200);
      if (error) return fail(`Cannot read your contacts: ${String(error.message).slice(0, 120)}.`, "Open /diagnostics — a table or column is probably missing.");
      const rows = data || [];
      if (!rows.length) return skip("No contacts saved yet.", "Run Find clients once, then run this check again.");
      let refusedSource = 0, refusedRole = 0, partners = 0, customers = 0;
      for (const c of rows) {
        if (!checkSource(c.source).ok) refusedSource++;
        if (roleFit(c.email, { purpose: "any" }).hard) refusedRole++;
      ...
      const detail = `${rows.length} contacts: ${sendable} sendable, ${refusedSource} refused on where the address was found, ${refusedRole} refused on the address itself (no-reply, legal, abuse). ${customers} read as buyers, ${partners} as agency partners.`;
