...
export const CHECKS = [
  ...
  {
    id: "markets", group: "The plan", label: "Each country gets its own share of the day",
    run: async (ctx) => {
      const s = needHost(ctx); if (s) return s;
      const { storedStrategy } = await import("@/lib/strategy-store");
      const { normalizeStrategy } = await import("@/lib/strategy");
      const { activeMarkets, nextMarket, marketNote } = await import("@/lib/market-plan");
      const { resolveMarket, volumeLabel } = await import("@/lib/geo-targets");
      const raw = await storedStrategy(ctx.supabase, ctx.userId);
      const names = raw ? (normalizeStrategy(raw).markets || []) : [];
      if (!names.length) {
        // Not a fault on a new account, but it IS the reason every search volume
        // reads as American, and an owner should know that before trusting one.
        return warn("No country is set, so search volumes are measured in the United States and articles are written for nowhere in particular.",
          "Open Market Testing and let it rank your countries, then press Rebuild on The plan.");
      }
      const unknown = names.filter((n) => !resolveMarket(n).known);
      const live = activeMarkets(names, 5);
      const next = nextMarket(live, {});
      const where = live.map((m) => `${m.name} (${m.tier})`).join(", ");
      const detail = `${live.length} live: ${where}. ${volumeLabel(names[0])}. Next article is written for ${next?.name || "—"}. ${marketNote(live[0])}`;
      return unknown.length
        ? warn(`${detail} Google has no country id for: ${unknown.join(", ")} — volumes for those fall back to the United States.`,
            "Use the country's full name as Market Testing writes it.")
        : pass(detail);
    },
  },
  {
  ...
  {
    id: "blog-index", group: "Connections", label: "Your blog page lists your articles",
    run: async (ctx) => {
      const s = needHost(ctx); if (s) return s;
      ...
      try {
        ...
        const { data } = await admin.from("published_pages")
          ...
          .eq("user_id", ctx.userId).eq("handle", ctx.host).eq("status", "published")
          .order("published_at", { ascending: false }).limit(100);
        rows = data || [];
      } catch (e) {
        return fail(`Cannot read your published articles: ${String(e?.message || e).slice(0, 120)}.`, "Open /diagnostics — a table or column is probably missing.");
      }
      if (!rows.length) return skip("No articles published yet.", "Approve an article in Approvals, then run this again.");

      // Fetched the way a reader fetches it. An article that is live but missing
      // from the index is invisible to everyone who does not already have the
      // link, which is everyone.
      let base = null;
      try { const { blogBaseFor } = await import("@/lib/own-blog"); base = await blogBaseFor(ctx.supabase, ctx.userId, ctx.host); } catch {}
      const indexUrl = base || `${String(process.env.APP_URL || "").replace(/\/+$/, "")}/b/${ctx.host}`;
      let listed = 0, err = null;
      try {
        const r = await fetch(`${indexUrl}?selftest=${Date.now()}`, { headers: { "User-Agent": "MarketingGenie-selftest/1.0" }, signal: AbortSignal.timeout(15000) });
        const html = r.ok ? await r.text() : "";
        listed = rows.filter((p) => p.slug && html.includes(p.slug)).length;
        if (!r.ok) err = `HTTP ${r.status}`;
      } catch (e) { err = String(e?.message || e).slice(0, 80); }

      if (err) return warn(`${rows.length} published, but the index could not be read from this server (${err}).`, "The articles themselves are live. Open that address in a browser to check by eye.");
      if (listed === 0) {
        return fail(
          `${rows.length} article${rows.length === 1 ? " is" : "s are"} published and live, but none appear on ${indexUrl}. Anyone browsing your blog sees an empty page.`,
          "They are reachable by direct link and are in your sitemap and RSS, so search engines still find them. The index page itself is the fault. Send this report to your developer."
        );
      }
      if (listed < rows.length) return warn(`${listed} of ${rows.length} published articles appear on ${indexUrl}.`, "The rest are reachable by link, but nobody browsing the blog will find them.");
      return pass(`All ${rows.length} published article${rows.length === 1 ? "" : "s"} appear on ${indexUrl}.`);
    },
  },
  {
  ...
  {
    id: "news-signals", group: "Finding customers", label: "News about a company, for the opening line",
    run: async (ctx) => {
      // This exists because the answer cannot be known from anywhere else. GDELT is
      // free and keyless and refuses shared cloud addresses: three attempts from
      // the developer's own machine, thirteen seconds apart, were all rejected.
      // Vercel's addresses are shared too, and this codebase has met exactly that
      // before with Reddit. One request from the real deployment settles it.
      const { newsReachable, recentSignal } = await import("@/lib/news-signals");
      const reach = await newsReachable();
      if (!reach.ok) {
        return warn(
          `Cannot reach the news service from this server${/rate_limited/.test(reach.reason) ? " — it is refusing this host's address" : ` (${reach.reason})`}.`,
          "Nothing is broken and nothing is missing. Cold emails open normally instead of with a line about something the company just did. It is free and needs no key, so the only fix is a different address to call from, which is not worth buying yet."
        );
      }
      // Reachable. Does it actually find anything for a company this owner would
      // write to? A working connection that returns nothing useful is worth
      // knowing about separately.
      const sig = await recentSignal("IKEA", { days: 60 });
      return pass(sig
        ? `Reachable, and it finds real news: "${String(sig.headline).slice(0, 90)}". Cold emails can open with something the company actually did.`
        : "Reachable. No recent news for the company used to test it, which is normal — most companies have none, and those emails just open normally.");
    },
  },
  {
