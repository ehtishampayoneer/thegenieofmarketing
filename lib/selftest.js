...
export const CHECKS = [
  ...
  // FINDING CUSTOMERS
  {
    ...
    run: async (ctx) => {
      ...
      const q = `best ${niche(ctx)}`;
      const r = await webSearch(q, { limit: 5 });
      return r.length ? pass(`"${q}" returned ${r.length} results, e.g. ${r[0].url}`) : fail(`"${q}" returned nothing.`, "Every finder depends on search. Gemini grounded search needs GEMINI_API_KEY; the free fallback is often blocked from servers.");
    },
  },
  {
  ...
  },
  {
    ...
    run: async () => {
      ...
      return v?.score ? pass(`wikipedia.org scores ${v.score}/10. Giants above 6 are now filtered out of Find clients.`) : fail("The key is set but no score came back.", "Check the key at openpagerank.com and its monthly quota.");
    },
  },
