...
export const CHECKS = [
  ...
  ...["gemini", "groq", "openrouter"].map((p) => ({
    ...
    run: async () => {
      ...
      try {
        const r = await callAI({ only: [p], json: true, maxTokens: 60, temperature: 0, prompt: 'Return exactly this JSON: {"ok":true}', timeoutMs: 30000 });
        return r.json?.ok ? pass(`Answered in ${r.latencyMs}ms (${r.model}).`) : warn(`Answered, but not with the JSON asked for.`, null, r.text?.slice(0, 200));
      } catch (e) {
  ...
  // FINDING CUSTOMERS
  {
    ...
    run: async (ctx) => {
      ...
      if (r.length) {
        ...
        return (why ? warn : pass)(`"${q}" returned ${r.length} results, e.g. ${r[0].url}`, why ? `Google search (Gemini) failed and a fallback answered instead: ${why}` : undefined);
      }
      return fail(`"${q}" returned nothing. Gemini search: ${why || "no error recorded"}.`, /429|quota|RESOURCE_EXHAUSTED/i.test(why || "") ? "Gemini's free Google Search allowance is used up for today. It resets daily; or add BRAVE_SEARCH_API_KEY as a second search source." : "Every finder depends on search. Send this report to your developer.");
    },
  },
  {
  ...
  },
  {
    ...
    run: async () => {
      ...
      return fail(why ? `OpenPageRank refused: ${why}` : "The key is set but no score came back.", /401/.test(why || "") ? "The key is not valid for the new API. Create an opr_live_ key at openpagerank.keywordseverywhere.com (old DomCop keys stop working 30 September 2026)." : "Check the key and its monthly quota at openpagerank.keywordseverywhere.com.");
    },
  },
