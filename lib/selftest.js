...
const AI_NAMES = { gemini: "Google Gemini", groq: "Groq", openrouter: "OpenRouter", mistral: "Mistral", github: "GitHub Models", cloudflare: "Cloudflare Workers AI", cerebras: "Cerebras" };
...
export const CHECKS = [
  ...
  ...["gemini", "groq", "openrouter", "mistral", "github", "cloudflare", "cerebras"].map((p) => ({
    id: `ai-${p}`, group: "AI", label: `AI provider: ${AI_NAMES[p]}`,
    run: async () => {
      const key = { gemini: "GEMINI_API_KEY", groq: "GROQ_API_KEY", openrouter: "OPENROUTER_API_KEY", mistral: "MISTRAL_API_KEY", github: "GITHUB_MODELS_TOKEN", cloudflare: "CLOUDFLARE_AI_TOKEN", cerebras: "CEREBRAS_API_KEY" }[p];
...

  {
    ...
    run: async (ctx) => {
      ...
      return keyed.length < 3
        ? warn(`Testing (last ${hours}h ago), but only ${capacity}. When they run out it switches to quick checks.`, "Add more free AI keys in Vercel for full-size tests all day: CEREBRAS_API_KEY, MISTRAL_API_KEY, GITHUB_MODELS_TOKEN, or CLOUDFLARE_ACCOUNT_ID + CLOUDFLARE_AI_TOKEN.")
