...
export function buildIntentQueries(entity, ai = {}, keywords = []) {
  ...
  const named = sorted.filter((x) => x.group !== "buyer");
  const out = [];
  while ((buyer.length || named.length) && out.length < 8) {
    if (buyer.length) out.push(buyer.shift());
    if (named.length && out.length < 8) out.push(named.shift());
  }
  return out;
}
