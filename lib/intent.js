...
import { growthQueries, awarenessOf } from "@/lib/buyer-angle";
...
export function buildIntentQueries(entity, ai = {}, keywords = []) {
  // ── A SELLER LOOKING FOR WAYS TO SELL MORE ──
  // Every query here used to assume the buyer was shopping for the product:
  // comparing names, asking for the best one, looking for an alternative. That
  // finds people only when they already know the category exists. Nobody
  // searches for an AR vendor — but a furniture retailer searches "how to
  // increase furniture store sales" and "best apps for my store" constantly,
  // because growing is their job. That is where this product has to appear, as
  // one of the ways, with the reason it works.
  const growth = growthQueries(ai, { limit: 6 });
  ...
  const growthQ = sorted.filter((x) => x.group === "growth");
  const buyer = sorted.filter((x) => x.group === "buyer");
  const named = sorted.filter((x) => x.group !== "buyer" && x.group !== "growth");
  // "alternative to X" finds nobody when X is a name buyers have never heard, so
  // when no rival was named the growth searches lead and the rest fill in.
  const order = awareness === "growth" ? [growthQ, buyer, named] : [buyer, growthQ, named];
  const out = [];
