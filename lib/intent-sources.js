...
// Stack Exchange search — default to Software Recommendations (softwarerecs), which is
// literally people asking "recommend software that does X" = pure buyer intent. Free,
// no key (throttled but plenty for this).
export async function stackExchangeSearch(query, { site = "softwarerecs", limit = 6 } = {}) {
  const data = await getJson(`https://api.stackexchange.com/2.3/search/advanced?order=desc&sort=relevance&q=${encodeURIComponent(query)}&site=${encodeURIComponent(site)}&pagesize=${Math.min(limit, 10)}&filter=default`);
  const items = data?.items || [];
  return items
    .filter((i) => i.title && i.link && relevant(i.title, query))
    .map((i) => ({ title: strip(i.title).slice(0, 180), url: i.link, snippet: `${i.answer_count || 0} answers · score ${i.score || 0}` }))
    .slice(0, limit);
}
