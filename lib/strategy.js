...
// opt-out. Canada needs the role check as well; Germany layers unfair-competition
// law on top of GDPR and expects a written assessment per contact, so both are
// left off until the product can honestly meet them. This is the general shape
// of those rules and not legal advice - see the rebuild map.
...
export function strategyBlock(s) {
  ...
  // The offer, because every engine argued the case and then stopped short of
  // asking for anything. The partner version is labelled separately: an agency
  // that builds twenty stores is not being sold a subscription.
  if (n.offer) lines.push(`What we sell, and what it costs: ${n.offer}`);
  if (n.partnerOffer) lines.push(`If you are writing to an AGENCY or a partner rather than a customer, this is what they get: ${n.partnerOffer}. Do not pitch them the customer price — they are not buying it, they are putting it in front of their own clients.`);
