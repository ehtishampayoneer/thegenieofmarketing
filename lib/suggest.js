...
export function targetSuggestions({ ai = {}, limit = 5 } = {}) {
  ...
  // What the owner told Genie in onboarding beats what the homepage implies.
  const told = (Array.isArray(ai.brief?.segments) ? ai.brief.segments : [])
    .map((s) => String(s).trim()).filter((p) => p && p.split(/\s+/).length <= 5);
  for (const p of told.slice(0, 4)) c.add(p, "who you told Genie to target");

  const parts = told.length ? told : clauses(ai.targetCustomer).filter((p) => p.split(/\s+/).length <= 5);
