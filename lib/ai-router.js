...
export async function callAI(opts = {}) {
  const {
    ...
    // ── THE WHOLE CALL'S WALL CLOCK ──
    // timeoutMs is per provider, and this loop tries several. That is the right
    // behaviour for reliability and it quietly breaks a caller's arithmetic: a
    // route that allows 48 seconds for writing, passing timeoutMs: 48000, could
    // spend 48 on a provider that hung and 48 more on the next — and be killed by
    // the host at 60 having produced nothing. Pass deadlineMs and the loop stops
    // trying when the time is gone, and shortens the last attempt to fit rather
  ...
  const hardDeadline = Number.isFinite(Number(deadlineMs)) && Number(deadlineMs) > 0
    ? startedAll + Number(deadlineMs)
    : null;
  // Below this there is not enough left for a model to answer, so the attempt
  // would only burn the remaining time and still fail.
  ...
  for (const provider of ordered) {
    ...
    // Any time left? An attempt that cannot finish is worse than no attempt: it
    // guarantees the host kills the function with nothing saved.
    let attemptMs = timeoutMs;
    if (hardDeadline) {
      const left = hardDeadline - Date.now();
      if (left < MIN_ATTEMPT_MS) {
        attempts.push({ provider: provider.name, skipped: "out_of_time", leftMs: Math.max(0, left) });
        logger.warn("ai.skip.deadline", { provider: provider.name, leftMs: Math.max(0, left) });
        break;
      }
      attemptMs = Math.min(timeoutMs, left);
    }
    const payload = { prompt, system, json, temperature, maxTokens, timeoutMs: attemptMs };
