...
export const FOUNDATION_SITES = [
  ...
  // ── JOURNALIST REQUESTS ──
  // The single best free source of genuine press links. Worth naming the current
  // ones explicitly because the landscape changed and most advice online is
  // stale: Cision retired HARO into Connectively, which shut down in late 2024.
  // These are the free ones that actually operate now. They are email digests,
  // so Genie cannot answer them for you: you subscribe, and when a request fits,
  // you reply. One good answer can earn a link from a national publication.
  { id: "sos", name: "Source of Sources", url: "https://sourceofsources.com/", cat: "Press", why: "Free, and run by the person who originally created HARO. Up to three digests of journalist requests a day. Answer the ones you genuinely know and you get quoted, with a link." },
  { id: "helpab2bwriter", name: "Help a B2B Writer", url: "https://helpab2bwriter.com/", cat: "Press", why: "Free. B2B only, so the requests are far more relevant if you sell to businesses, and there is much less competition per request than the general services." },
  { id: "sourcebottle", name: "SourceBottle", url: "https://www.sourcebottle.com/", cat: "Press", why: "Free. Strong for consumer, lifestyle and local stories, and for smaller publications that still link generously." },
  { id: "featured", name: "Featured", url: "https://featured.com/", cat: "Press", why: "Free tier. Editors post questions, experts answer, the best answers get published with attribution. Now also runs the revived HARO newsletter." },
  ...
  {
    id: "chromestore", name: "Chrome Web Store", url: "https://chrome.google.com/webstore/devconsole/register", cat: "Brand",
    why: "Publish one simple, functional extension and your developer listing links to your site — a brand touchpoint on a DR-99 domain. The link is nofollow, so treat it as brand + referral, not a ranking hack.",
    guide: [
      "Register once at the Chrome Web Store developer console (a $5 one-time Google fee).",
      "Make a folder with the three files below: manifest.json, popup.html, popup.js.",
      "In Chrome open chrome://extensions, turn on Developer mode, click 'Load unpacked', and pick the folder to test it works.",
      "Zip the folder, upload it in the developer console, and fill the listing (title, description, icon, screenshots).",
      "In the listing's 'Website' field, put YOUR site URL — that is the backlink.",
      "Submit for review (Google reviews manually; it can take days). Keep it genuinely useful, never a link-only extension.",
    ],
    snippet: {
      "manifest.json": `{
  "manifest_version": 3,
  "name": "Quick Notes",
  "version": "1.0",
  "description": "A tiny scratchpad in your toolbar. Jot a note, it saves locally.",
  "permissions": ["storage"],
...
<html>
  <head>
    <meta charset="utf-8" />
    <style>
      body { width: 240px; margin: 0; font-family: system-ui, sans-serif; }
      textarea { width: 100%; height: 160px; border: 0; padding: 10px; box-sizing: border-box; resize: none; font: 14px system-ui; }
    </style>
  </head>
  <body>
    <textarea id="n" placeholder="Type a quick note..."></textarea>
    <script src="popup.js"></script>
  </body>
</html>`,
      "popup.js": `const n = document.getElementById("n");
chrome.storage.local.get("note", (r) => { n.value = r.note || ""; });
n.addEventListener("input", () => chrome.storage.local.set({ note: n.value }));`,
    },
  },
];
