...
import { setGenieContext } from "@/lib/context";
...
export async function resolveRadarUser(request, body) {
  ...
  if (cronHeader && process.env.CRON_SECRET && cronHeader === process.env.CRON_SECRET && body?._uid) {
    ...
    setGenieContext({ supabase, userId: body._uid, host: body?.host || null });
    return { supabase, userId: body._uid, cron: true };
  }
  const supabase = createClient();
  const { data: { user } } = await supabase.auth.getUser();
  setGenieContext({ supabase, userId: user?.id || null, host: body?.host || null });
