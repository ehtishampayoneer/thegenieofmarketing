...
// components/team/LaunchTest.js
// ── TEST IT BEFORE YOU LAUNCH ──
// Lives inside "Your team" (the Test something tab), because it is the same
// crowd: the testers who read every draft overnight, pointed at one thing on
// demand. Kept as its own component so the team page stays readable.
...
export default function LaunchTest() {
  ...
  // The countries Genie already ranked for this business (Market testing), so
  // picking a market is a choice from real data rather than a guess.
  ...
  useEffect(() => {
    ...
    fetch("/api/markets", { cache: "no-store" }).then((r) => r.json()).then((j) => {
      if (!j?.ok) return;
      const started = (j.experiments || []).map((e) => ({ name: e.name, flag: e.flag, note: "being tested" }));
      const ranked = (j.rows || []).slice(0, 12).map((r) => ({ name: r.name, flag: r.flag, note: r.difficulty ? `${r.difficulty.toLowerCase()} to win` : "" }));
      const seen = new Set();
      setCountries([...started, ...ranked].filter((c3) => c3.name && !seen.has(c3.name) && seen.add(c3.name)));
    }).catch(() => {});
  }, []);
    ...
    <>
      ...
      <div className="mt-5 rounded-2xl p-5" style={{ background: "var(--surface)", border: "1px solid var(--hair)" }}>
        ...
        <div className="mt-3 grid gap-3" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))" }}>
          ...
          <label className="text-[12.5px] mg-muted">Which market? (optional)
            <select value={market} onChange={(e) => setMarket(e.target.value)} className="mg-field mg-focus mt-1 w-full" style={{ fontSize: 13 }}>
              <option value="">Everyone, wherever they are</option>
              {countries.map((c2) => <option key={c2.name} value={c2.name}>{c2.flag ? `${c2.flag} ` : ""}{c2.name}{c2.note ? ` — ${c2.note}` : ""}</option>)}
            </select>
            <span className="mt-1 block text-[11.5px] mg-subtle">
              {countries.length
                ? <>Ranked for you in <a href="/markets" style={{ color: "var(--accent-ink)" }}>Market testing</a>, best opportunities first.</>
                : <>Genie ranks countries worth testing in <a href="/markets" style={{ color: "var(--accent-ink)" }}>Market testing</a>.</>}
            </span>
          </label>
        </div>
