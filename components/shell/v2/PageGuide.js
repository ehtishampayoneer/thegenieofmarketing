          ...
          <div className="mg-rise" style={{ width: "100%", maxWidth: 580, maxHeight: "88vh", overflowY: "auto", background: "var(--surface)", border: "1px solid var(--border-strong)", borderRadius: 20, boxShadow: "var(--shadow-3)" }}>
            ...
            <div style={{ display: "flex", alignItems: "flex-start", gap: 13, padding: "22px 22px 0" }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <p style={{ fontSize: 12, fontWeight: 700, letterSpacing: ".13em", textTransform: "uppercase", color: "var(--accent-ink)" }}>How this page works</p>
                <h2 style={{ fontSize: 23, fontWeight: 700, color: "var(--fg)", margin: "3px 0 0", lineHeight: 1.12, letterSpacing: "-.01em" }}>{guide.name}</h2>
              </div>
            ...
            <p style={{ padding: "12px 22px 0", fontSize: 15, color: "var(--fg-muted)", lineHeight: 1.55, maxWidth: "var(--measure)" }}>{guide.what}</p>
            ...
            {Array.isArray(guide.use) && guide.use.length > 0 && (
              <div style={{ margin: "16px 22px 0", padding: "15px 17px", borderRadius: 14, background: "var(--accent-quiet)", border: "1px solid var(--accent)" }}>
                <p style={{ fontSize: 12, fontWeight: 700, letterSpacing: ".08em", textTransform: "uppercase", color: "var(--accent-ink)", margin: "0 0 11px", display: "flex", alignItems: "center", gap: 6 }}><Icon.check size={15} /> How to use this page</p>
                <ol style={{ margin: 0, padding: 0, listStyle: "none", display: "flex", flexDirection: "column", gap: 11 }}>
                  {guide.use.map((u, i) => (
                    <li key={i} style={{ display: "flex", gap: 11, alignItems: "flex-start" }}>
                      <span style={{ flexShrink: 0, width: 22, height: 22, borderRadius: 999, background: "var(--accent)", color: "var(--on-accent)", fontSize: 12, fontWeight: 700, display: "grid", placeItems: "center", fontFamily: "var(--font-mono, monospace)", marginTop: 1 }}>{i + 1}</span>
                      <span style={{ fontSize: 15, color: "var(--fg)", lineHeight: 1.5 }}>{u}</span>
                    </li>
                  ))}
                </ol>
              </div>
            ...
            <p style={{ padding: "20px 22px 0", margin: 0, fontSize: 12, fontWeight: 700, letterSpacing: ".08em", textTransform: "uppercase", color: "var(--fg-subtle)" }}>What Genie does behind the scenes</p>
                  ...
                  <div key={i} style={{ display: "grid", gridTemplateColumns: "38px 1fr", gap: 14, alignItems: "stretch" }}>
                    <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
                      <span style={{ position: "relative", width: 38, height: 38, flexShrink: 0, borderRadius: 11, background: "var(--surface-sunken)", border: "1px solid var(--hair)", color: "var(--fg)", display: "grid", placeItems: "center" }}>
                        <Si size={18} />
                        <span style={{ position: "absolute", top: -6, right: -6, width: 18, height: 18, borderRadius: 999, background: "var(--primary)", color: "var(--on-primary)", fontSize: 11, fontWeight: 700, display: "grid", placeItems: "center", fontFamily: "var(--font-mono, monospace)" }}>{i + 1}</span>
                      </span>
                      {!last && <span style={{ width: 2, flex: 1, minHeight: 14, background: "var(--hair)", margin: "5px 0" }} />}
                    </div>
                    <div style={{ paddingBottom: last ? 2 : 16 }}>
                      <p style={{ fontSize: 16, fontWeight: 600, color: "var(--fg)", margin: 0 }}>{s.title}</p>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, padding: "10px 22px 0" }}>
              <div style={{ background: "var(--accent-quiet)", borderRadius: 12, padding: "13px 14px" }}>
                <p style={{ fontSize: 12, fontWeight: 700, letterSpacing: ".07em", textTransform: "uppercase", color: "var(--accent-ink)", margin: "0 0 6px", display: "flex", alignItems: "center", gap: 5 }}><Icon.spark size={14} /> Genie does</p>
                <p style={{ fontSize: 14, color: "var(--fg)", margin: 0, lineHeight: 1.5 }}>{guide.genie}</p>
              </div>
              <div style={{ background: "var(--surface-sunken)", borderRadius: 12, padding: "13px 14px" }}>
                <p style={{ fontSize: 12, fontWeight: 700, letterSpacing: ".07em", textTransform: "uppercase", color: "var(--fg-muted)", margin: "0 0 6px", display: "flex", alignItems: "center", gap: 5 }}><Icon.check size={14} /> You do</p>
                <p style={{ fontSize: 14, color: "var(--fg)", margin: 0, lineHeight: 1.5 }}>{guide.you}</p>
              </div>
            </div>
            ...
            <div style={{ margin: "16px 22px", padding: "14px 16px", borderRadius: 12, background: "var(--signal-live-soft)", border: "1px solid var(--signal-live)", display: "flex", gap: 11, alignItems: "flex-start" }}>
              ...
              <div>
                <p style={{ fontSize: 12, fontWeight: 700, letterSpacing: ".07em", textTransform: "uppercase", color: "var(--signal-live-ink)", margin: "0 0 4px" }}>Why it makes you money</p>
